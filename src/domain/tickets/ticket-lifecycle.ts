...
export const allowedTicketTransitions: Record<TaskStatus, readonly TaskStatus[]> = {
...
export function canTransitionTaskStatus(current: TaskStatus, next: TaskStatus): boolean {
  return current === next || allowedTicketTransitions[current].includes(next);
}
...
export function settleTaskStatus(task: Task, nextStatus: TaskStatus, at = new Date()): Task {
  if (canTransitionTaskStatus(task.status, nextStatus)) {
    return transitionTask(task, nextStatus, at);
  }
